      ...
      <div class="content">
        <div v-if="!showDetail" class="contanternews">
          ...
          <div class="selectChannel backWhite padding20 baseInfo">
            ...
            <div class="lableBox">
              <span class="colorGrey lableText font12 marginright10"><span class="required">*</span>商品名称</span>
              <input type="text" class="colorblack font12 inputBox" v-model="goodsObj.name" @change="changeName('name')"
                     placeholder="请输入商品名称（30字以内）"/>
            </div>
            <div class="lableBox">
              <span class="colorGrey lableText font12 marginright10">商品卖点</span>
              <input type="text" class="colorblack font12 inputBox" v-model="goodsObj.sellingPoint"
                     @change="changeName('sellingPoint')"
                     placeholder="请输入商品卖点"/>
              <span class="font12 colorGrey">60字以内</span>
              <span class="font12 colorWhite">?</span>
            </div>
            <div class="lableBox">
              <span class="colorGrey lableText font12 marginright10"><span class="required"
                                                                           v-if="goodsObj.goodsType == 2">*</span>归属渠道</span>
              <el-select v-model="goodsObj.supplierId" placeholder="选择归属渠道">
                <el-option
            ...
            <div class="lableBox">
              <span class="colorGrey lableText font12 marginright10"><span class="required"
                                                                           v-if="goodsObj.goodsType == 2">*</span>渠道商品编码</span>
              <input type="text" class="colorblack font12 inputBox" v-model="goodsObj.supplierProductCode"
                     @change="changeName('supplierProductCode')"
                     placeholder="请输入归属渠道商品编码"/>
            </div>
            <div class="lableBox flex" style="margin-bottom: 0;">
              <span class="colorGrey lableText font12 marginright10" style="display: inline-block;width: 64px;"><span
                class="required">*</span>商品图</span>
          ...
          <div class="selectChannel backWhite padding20 baseInfo">
            ...
            <div class="lableBox flex" v-if="specs.length != 0">
              <span class="colorGrey font12 marginright10 lableText">规格明细</span>
                ...
                <tr v-for="item in specs">
                  ...
                  <td>
                    ...
                    <input type="tel" class="editorInput borderBox" v-model="item.marketPrice"
                           @blur="removsellMarketPriceBorder(item,'')">
                  </td>
                  <td>
                  ...
                  <td>
                    <input type="tel" class="editorInput borderBox" v-model="item.code"
                           style="width: 150px"
                           @blur="removmarketspeCodeBorder(item,'')">
                  </td>
                </tr>
              </table>
            </div>
            <div class="lableBox">
              ...
              <span class="colorGrey font12 marginright10">下单锁定库存，买家支付成功，扣减库存数量</span>
            </div>
            <div class="lableBox">
              <span class="colorGrey font12 lableText marginright10"><span class="required">*</span>库存</span>
              <input type="text" readonly class="colorblack readonlyBox font12 inputBox yunfeibox" v-model="stock"
                     placeholder=""/>
              <span class="colorGrey font12 marginLeft10">库存为0时，显示为已售馨完成</span>
            </div>
            <div class="lableBox">
              <span class="colorGrey font12 marginright10 lableText">积分消耗方式</span>
              <span class="colorGrey font12 marginright10">直接扣除积分</span>
            </div>
          </div>
          <div class="selectChannel backWhite padding20 baseInfo">
            <div class="channnelTitle">
            ...
            <div class="lableBox">
              <span class="colorGrey lableText font12 marginright10"><span class="required">*</span>运费</span>
              <input type="text" class="colorblack font12 inputBox" v-model="goodsObj.shippingFee"
                     @change="changeName('shippingFee')"
            ...
            <div class="lableBox">
              ...
              <input type="text" class="colorblack font12 inputBox" v-model="goodsObj.shippingExplain"
                     @change="changeName('shippingExplain')"
                     placeholder="请输入小于30个字符"/>
            </div>
          </div>
        ...
        <Lzcommoditydetail :goodsObj="goodsObj" :productId="productId" v-if="showDetail"
                           @clickbanner="showDetail = false"></Lzcommoditydetail>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  import Lzcommoditydetail from '@/components/lzcommoditydetail'
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 商品信息
        supplierIdOptions: [{name: '（空）', value: 0}],
        goodsObj: {
          goodsType: 1,
          name: '', //名称
          sellingPoint: '', //卖点
          marketPrice: '', //划线价
          supplierProductCode: '', //商品编码
          supplierId: '', //归属渠道
          shippingFee: 0,
          shippingExplain: '不支持发货的区域：西藏，新疆，台湾，香港，澳门以及国外',
          imgs: [],
          imgUrl: '',
          specs: [], // 规格数组
          description: '',//产品详情
    ...
    mounted() {
      ...
      this.getsupplierIdOptions() // 获取归属渠道
    ...
    methods: {
      ...
      getCommodity(productId) {
        Service.fxjactivity().getgoodsDetail({}, productId
          ...
          if (response.errorCode == 0) {
            ...
            // this.selectData = this.goodsObj.goodsType == 1
            //   ? [{'name': '实物商品', isSelect: true, postType: '物流发货',}, {
            //     'name': '虚拟卡服务',
            //     isSelect: false,
            //     postType: '卡密',
            //   }]
            //   : [{'name': '实物商品', isSelect: false, postType: '物流发货',}, {
            //     'name': '虚拟卡服务',
            //     isSelect: true,
            //     postType: '卡密',
            //   }]
            ...
            this.goodsObj.supplierId = response.data.supplierId;
            this.goodsObj.sellingPoint = response.data.sellingPoint;
            this.goodsObj.supplierProductCode = response.data.supplierProductCode;
            if (response.data.specs) {
              for (var i = 0; i < response.data.specs.length; i++) {
                ...
                var obj = {
                  ...
                  code: response.data.specs[i].code,
                };
                this.specs.push(obj)
              }
            }
      ...
      changeName(str, type) {
        ...
        if (str == 'supplierId' && (this.goodsObj.goodsType == 2)) {
          if (!this.goodsObj[str]) {
            this.$message.error('请选择归属渠道');
            on = false;
            return;
          }
        }
        if (str == 'supplierProductCode' && (this.goodsObj.goodsType == 2)) {
          if (this.goodsObj[str].length < 1) {
            this.$message.error('归属渠道商品编码必须填写');
            on = false;
            return;
          }
        }
        if (str == 'sellingPoint') {
          if (this.goodsObj[str].length > 60) {
            this.$message.error('商品卖点最多60个字');
            on = false;
            return;
          }
        }
        ...
        if (str == 'shippingExplain') {
          if (this.goodsObj[str].length > 60 || this.goodsObj[str].length < 2) {
            this.$message.error('配送描述必须填写，最多60个字且不少于2个字');
            on = false;
            return;
          }
        }
      ...
      getsupplierIdOptions() {
        // 获取归属渠道
        Service.fxjactivity().getsupplier({}).then(response => {
          if (response.errorCode == 0) {
            let transArr = response.data.map(item => {
              return {
                name: item.name,
                value: item.id,
              }
            })
            this.supplierIdOptions = [...this.supplierIdOptions,...transArr]
          } else {
            this.$message.error(response.message);
          }
        })
      ...
      goDetail() {
        ...
        if (!this.changeName('supplierId', 'submit')) {
          return;
        }
        if (!this.changeName('supplierProductCode', 'submit')) {
          return;
        }
        ...
        if (this.specs.length != 0) {
          for (var i = 0; i < this.specs.length; i++) {
            ...
            if (!this.removsellMarketPriceBorder(this.specs[i], 'submit')) {
              return;
            }
            ...
            if ((this.goodsObj.goodsType == 2) && !this.removmarketspeCodeBorder(this.specs[i], 'submit')) {
              return;
            }
          }
        }
      ...
      addSpec() {
        ...
        this.specs.push(obj)
        this.goodsObj.price = this.specs[0].price;
        this.goodsObj.marketPrice = this.specs[0].marketPrice;
        this.stock = 0;
        for (var i = 0; i < this.specs.length; i++) {
      ...
      removsellMarketPriceBorder(item, type) {
        var on = true;
        ...
        for (let i in this.specs) {
          if (parseFloat(this.specs[i].price) >= parseFloat(item.marketPrice)) {
      ...
      removmarketspeCodeBorder(item, type) {
        var on = true;
        if ((this.goodsObj.goodsType == 2) && !item.code) {
          on = false;
          this.$message.error('请输入规格编码');
          return
        }
        if (type == 'submit') {
          return on;
