    ...
    <div class="salecustomercontent contaner">
      ...
      <div class="content">
        <div class="contanternews">
          <div class="dataGeneral backWhite" v-show="groupPurchaseView">
          ...
          <div class="dataGeneral bannerTable">
            <div class="itemscenter">
              <div class="borderButton cursor" v-show="groupPurchaseAdd" @click="addCommodity()">新建拼团活动</div>
            </div>
            <table v-if="noData&&groupPurchaseView" class="table">
              <tr>
              ...
              <tr v-for="(item,index) in tableData" :key="item.id" :inde="index">
                ...
                <td>
                  {{item.completedGroupCount}}
                </td>
                <td>
                  {{item.completedGroupUserCount}}
                </td>
                <td>
                  ...
                  <span class="color2087 font12 cursor fontWeight" 
                        v-show="groupPurchaseUpdate" @click="goDetail(item.id)">编辑</span>
                  <span class="line" v-show="groupPurchaseUpdate"></span>
                  <span class="color2087 font12 cursor fontWeight" @click="undercarriage(item.id)"
                        v-if="(item.status == 0)" v-show="groupPurchaseInvalid">使失效</span>
                  <span class="line" v-if="(item.status  == 0)" v-show="groupPurchaseInvalid"></span>
                  <span class="color2087 font12 cursor fontWeight" v-show="groupPurchaseInvalid"
                        @click="enableGoods(item.id)" v-if="(item.status == 1)">使生效</span>
                  <span class="line" v-if="(item.status == 1)" v-show="groupPurchaseInvalid"></span>
                  <span class="color2087 font12 cursor fontWeight" v-show="groupPurchaseCopy"
                        @click="copyGoods(item.id)">复制</span>
                  <span class="line" v-show="groupPurchaseCopy"></span>
                  <span class="color2087 font12  cursor fontWeight" v-show="groupPurchaseDelete"
  ...
  export default {
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: '', name: '拼团完成数', showBlue: false, orderField: ''},
          {orderType: '', name: '拼团完成人数', showBlue: true, orderField: ''},
        ...
        userInfo: '',
        permissions: [],
        groupPurchaseView: false,
        groupPurchaseAdd: false,
        groupPurchaseUpdate: false,
        groupPurchaseDelete: false,
        groupPurchaseInvalid: false,
        groupPurchaseCopy: false
      };
    },
    created() {
      this.userInfo = JSON.parse(localStorage.getItem('user'));
      this.groupPurchaseView = this.havePermissions('groupPurchase:view')
      this.groupPurchaseAdd = this.havePermissions('groupPurchase:add')
      this.groupPurchaseUpdate = this.havePermissions('groupPurchase:update')
      this.groupPurchaseDelete = this.havePermissions('groupPurchase:delete')
      this.groupPurchaseInvalid = this.havePermissions('groupPurchase:operate')
      this.groupPurchaseCopy = this.havePermissions('groupPurchase:operate')
      if (this.userInfo) {
      }
    ...
    mounted() {
      if (Global.env == 'dev' || Global.env == 'test') {
        this.httpurl = 'https://testhealthmall.ifxj.com/group/grouplist'
      } else {
        this.httpurl = 'https://healthmall.ifxj.com/group/grouplist'
      }
      if(this.groupPurchaseView){
    ...
    methods: {
      ...
      copyactivity(id) {
        Service.groupPurchase().copygroupPurchase({}, id).then(response => {
      ...
      sortRecData(firstId, secondId) {
        ...
        Service.groupPurchase().sortactivity({}, str).then(response => {
          if (response.errorCode == 0) {
      ...
      enableGoods(id) {
        this.$confirm('确认使活动生效?', '', {
          ...
          Service.groupPurchase().enablegroupPurchase({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '操作成功!'
      ...
      undercarriage(id) {
        this.$confirm('确认使活动失效', '', {
          ...
          Service.groupPurchase().disablegroupPurchase({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '操作成功!'
      ...
      delectCommodity(id) {
        this.$confirm('活动删除请谨慎操作，确定删除?', '', {
          ...
          Service.groupPurchase().deletegroupPurchase({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      addCommodity() {
        this.$router.push({name: 'addmultigroup'})
      },
      goDetail(id) {
        this.$router.push({name: 'addmultigroup', query: {id: id}})
      },
      ...
      getcommodityData(str) {
        ...
        Service.groupPurchase().querygroupPurchase({
          size: this.size,
          page: this.page,
          status: this.status,
          type: this.type
