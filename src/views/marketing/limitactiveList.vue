        ...
        <div class="contanternews">
          ...
          <div class="dataGeneral bannerTable">
            <div class="itemscenter">
              <div class="borderButton cursor" v-show="flashSaleAdd" @click="addCommodity()">新建限时抢购</div>
              <span class="colorGrey marginLeft10">仅能设置一个尚未开始的活动</span>
            </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: '', name: '付款实付金额', showBlue: true, orderField: ''},
    ...
    mounted() {
      if (Global.env == 'dev' || Global.env == 'test') {
        this.httpurl = 'https://testhealthmall.ifxj.com/limitshop/limitshop-list'
      } else {
        this.httpurl = 'https://healthmall.ifxj.com/limitshop/limitshop-list'
      }
    ...
    methods: {
      ...
      copyactivity(id) {
        Service.flashSale().copyactivity({}, id).then(response => {
      ...
      sortRecData(firstId, secondId) {
        ...
        Service.flashSale().sortactivity({}, str).then(response => {
          if (response.errorCode == 0) {
      ...
      enableGoods(id) {
        this.$confirm('确认使活动生效?', '', {
          ...
          Service.flashSale().invalidactivity({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '操作成功!'
      ...
      delectCommodity(id) {
        this.$confirm('活动删除请谨慎操作，确定删除?', '', {
          ...
          Service.flashSale().deleteactivity({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      getcommodityData(str) {
        ...
        Service.flashSale().queryactivity({
          size: this.size,
          page: this.page,
