        ...
        <div class="contanternews">
          ...
          <div class="dataGeneral bannerTable">
            <div class="itemscenter">
              <div class="borderButton cursor" v-show="bigTurntableAdd" @click="addCommodity()">新建抽奖活动</div>
            </div>
            <table v-if="noData&&bigTurntableView" class="table">
              ...
              <tr v-for="(item,index) in tableData" :key="item.id" :inde="index">
                ...
                <td>
                  <span class="color2087 cursor fontWeight"  v-show="bigTurntableInvalid" @click="goparticipantsNums(item.id)">查看</span>
                </td>
                <!-- <td>
                ...
                <td>
                  ...
                  <span class="color2087 font12 cursor fontWeight" v-if="item.status != 2 || item.status != 4"
                        v-show="bigTurntableUpdate" @click="goDetail(item.id)">编辑</span>
                  <span class="line" v-if="item.status != 2 || item.status != 4" v-show="bigTurntableUpdate"></span>
                  <span class="color2087 font12 cursor fontWeight" @click="undercarriage(item.id)"
                        v-if="(item.status != 4)" v-show="bigTurntableInvalid">使失效</span>
                  <span class="line" v-if="(item.status != 4)" v-show="bigTurntableInvalid"></span>
                  <span class="color2087 font12 cursor fontWeight" v-show="bigTurntableInvalid"
                        @click="enableGoods(item.id)" v-if="(item.status == 4)">使生效</span>
                  <span class="line" v-if="(item.status == 4)" v-show="bigTurntableInvalid"></span>
                  <span class="color2087 font12 cursor fontWeight" v-show="bigTurntableCopy"
                        @click="copyGoods(item.id)">复制</span>
                  <span class="line" v-show="bigTurntableCopy"></span>
                  <span class="color2087 font12  cursor fontWeight" v-show="bigTurntableDelete"
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.bigTurntableView = this.havePermissions('bigTurntable:view[shopId=2]')
      this.bigTurntableAdd = this.havePermissions('bigTurntable:add[shopId=2]')
      this.bigTurntableUpdate = this.havePermissions('bigTurntable:update[shopId=2]')
      this.bigTurntableDelete = this.havePermissions('bigTurntable:delete[shopId=2]')
      this.bigTurntableInvalid = this.havePermissions('bigTurntable:operate[shopId=2]')
      this.bigTurntableCopy = this.havePermissions('bigTurntable:operate[shopId=2]')
      this.bigTurntablequeryGoods = this.havePermissions('bigTurntable:queryGoods[shopId=2]')
      if (this.userInfo) {
      }
    ...
    methods: {
      goparticipantsNums(id){
         this.$router.push({name: 'luckdrawparticipants', query: {id: id,shopId: 2}})
      },
      Decode(arr) {
      ...
      addCommodity() {
        this.$router.push({name: 'addluckdraw',query: {shopId: 2}})
      },
      goDetail(id) {
        this.$router.push({name: 'addluckdraw', query: {id: id,shopId: 2}})
      },
