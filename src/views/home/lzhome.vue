        ...
        <div>
          <el-dialog title="下载报表" class="baobiao" :visible.sync="dialogFormVisible" center>
            <div class="flex font14">
              ...
              <div class="searchBox" v-if="shelvesType == 1" style="margin-top: 26px;">
                <el-date-picker
        ...
        <div class="contanternews">
          <div class="dataGeneral backWhite padding20" v-if="showOverview">
            <div class="dataGenetalTitle flex flexBetween">
              <div class='flextitle'>
                <span class="colorblack font16 fontWeight marginright10">乐兹概况</span>
                <span class="colorGrey font12">{{today}}</span>
              </div>
              <div v-show="overviewexport" class="downLoadButton cursor" @click="dialogFormVisible = true"><i
            ...
            <div class="dataGenetalContent">
              ...
              <table class="tableBox">
                ...
                <tr v-for="item in dataTable" :key="item.time">
                  ...
                  <td>
                    <span v-if=" item.orderPoints">{{item.orderPoints}} </span>
                    <span v-if=" !item.orderPoints">0</span>
                  </td>
                </tr>
              </table>
            </div>
          ...
          <div class="dataGeneral  backWhite padding20 waitThing " v-show="showtrend">
            <div class="dataGenetalTitle">
              <span class="colorblack font16 fontWeight marginright10">乐兹数据趋势图</span>
            </div>
            ...
            <div id='analysis' v-show="showtrend"></div>
          </div>
          <div class="dataGeneral   waitThing flex" v-if="showtodo">
            <div class="waitThingLeft backWhite padding20">
              <div class="dataGenetalTitle">
                <span class="colorblack font16 fontWeight marginright10">乐兹待办事项</span>
              </div>
              <div class="flexWrap">
                <div class="waitBox cursor" @click="lzorderlist(1)">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: 'desc', name: '兑换人数', showspan: false, orderField: ''},
          {orderType: 'desc', name: '订单数(个)', showspan: false, orderField: ''},
          {orderType: 'desc', name: '实际消耗积分', showspan: false, orderField: ''}
        ],
        selectday: 1,
        timeOption: [
          {name: '最近30天', value: 1},
        ...
        overviewexport: false, // 下载报表
      };
    },
    created() {
      this.today = this.getDay(0, 'getday');
      localStorage.setItem('shopId', 2)
      this.showOverview = this.havePermissions('home:overview[shopId=2]')
      this.showtrend = this.havePermissions('home:trend[shopId=2]')
      this.showtodo = this.havePermissions('home:todo[shopId=2]')
      this.overviewexport = this.havePermissions('home:overview:export[shopId=2]')
    },
    watch: {
      'selectday': function () {
        if (this.selectday) {
    ...
    methods: {
          ...
          if (response.errorCode == 0) {
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              if (this.selectnorm == 4) {
                ...
                y1data.push(data[i].orderPoints);
              }
            }
            if (this.selectnorm == 4) {
      ...
      lzorderlist(statius) {
        this.$router.push({'name': 'lzorderlist', query: {statius,}});
      },
