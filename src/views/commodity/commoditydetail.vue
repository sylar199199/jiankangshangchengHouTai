                ...
                <div class="contanternews">
                    ...
                    <div class="selectChannel backWhite padding20 baseInfo">
                        ...
                        <div class="commonProblem" v-if="isDetail==1">
                            <div class="problemBox" v-for="(item,index) in goodsObj.faqs" :key="item.id">
                                <div class="question">
                    ...
                    <div class="nextButoon flex" v-if="isDetail==0">
                        <span class="bacButton marginLeft10" @click="submitCommodity()">完成</span>
                    </div>
                    <div class="nextButoon flex" v-if="isDetail==1">
    ...
    export default {
            ...
            if(this.$route.query){
                ...
                if(!this.productId){
                    ...
                    if(Util.localStorageUtil.get('goodsObj'+this.productId)){
                        ...
                        this.guigeData = Util.localStorageUtil.get('guigeData'+this.productId);
                        console.log(Util.localStorageUtil.get('guigeData'+this.productId))
        ...
        methods:{
                        ...
                        if(response.data.skuList){
                            for(var i =0;i < response.data.skuList.length;i++){
                                var obj = {
                                    ...
                                    originalsellPrice: response.data.skuList[i].planPrice,
            ...
            submitCommodity(){
                ...
                if(this.goodsObj.faqs.length == 0){
                    this.$message.error('请输入常见问题');
                    return;
                }else{
                    for(var i = 0;i<this.goodsObj.faqs.length;i++){
                        if(this.goodsObj.faqs[i]){
                            if(!this.changeText('question',this.goodsObj.faqs[i],'submit')){
                                return;
                            }
                            if(!this.changeText('answer',this.goodsObj.faqs[i],'submit')){
                                return;
                            }
                        }
                    }
                ...
                for(var i = 0;i<this.goodsObj.faqs.length;i++){
                    if(this.goodsObj.faqs[i]){
                        this.goodsObj.faqs[i].sort = parseInt(i)+1;
                    }
                }
                this.goodsObj.channelId = 1;
                if(this.productId){
                    Service.goods().editorGoods(
                        ...
                        if(response.errorCode == 0){
                            ...
                            Util.localStorageUtil.clear('guigeData'+this.productId);
                    ...
                    Service.goods().addGoods(
                        ...
                        if(response.errorCode == 0){
                            ...
                            Util.localStorageUtil.clear('guigeData');
                            this.$message.success('操作成功');
                            this.$router.push({name: 'commoditymanage'})
