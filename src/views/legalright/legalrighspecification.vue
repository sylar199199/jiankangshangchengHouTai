...
<template>
  <div class="general">
    ...
    <div class="salecustomercontent contaner">
        ...
        <div class="contanternews">
          <div class="dataGeneral backWhite">
            <div class="searchContent flex clear">
              ...
              <div class="bacButton cursor" v-show="rightsPlanView" @click="getSalesspecification('search')">查询</div>
            </div>
          ...
          <div class="dataGeneral bannerTable">
            <table v-if="noData" class="table">
              <tr>
                <th v-for="item in sortDatas" v-show="rightsPlanView">
                  {{item.name}}
              ...
              <tr v-for="(item,index) in tableData">
                ...
                <td>
                  <span v-if="!item.serviceTypeName">-</span>
                  <span v-if="item.serviceTypeName"> {{item.serviceTypeName}}</span>
                </td>
                <td><span class="color2087 font12 cursor" v-show="rightsPlanServicebind" @click='openBindserver(item)'>绑定服务</span></td>
              </tr>
            </table>
            ...
            <div class="block" v-if="noData" v-show="rightsPlanView">
              <el-pagination
      ...
      <Bindserver v-if="showBindserver" @clickbanner="getbackData" :selectData="selectData"></Bindserver>
    </div>
  </div>
</template>
...
<script>
  ...
  import Bindserver from '@/components/bindserver'
  ...
  export default {
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: 'desc', name: '绑定服务类型', showBlue: false, orderField: 'planSalEndDate'},
          {orderType: 'desc', name: '操作 ', showBlue: false, orderField: ''}
        ],
        servicetypedata: [],
        insuranceDate: '',
        endDate: '',
        ...
        productPlanName: '',
        productPlanNo: '',
        rightsPlanView: false,
        rightsPlanServicebind: false,
      };
    },
    created() {
      this.userInfo = JSON.parse(localStorage.getItem('user'));
      this.rightsPlanView = this.havePermissions('rightsPlan:view')
      this.rightsPlanServicebind = this.havePermissions('rightsPlan:servicebind')
      if (this.userInfo) {
      }
    ...
    methods: {
      openBindserver(item) {
        this.selectData = {
          palnId: item.id,
          serviceProductIdLevel1: item.serviceProductIdLevel1 ? item.serviceProductIdLevel1 : '',
          serviceProductIdLevel2: item.serviceProductIdLevel2 ? item.serviceProductIdLevel2 : '',
          serviceProductIdLevel3: item.serviceProductIdLevel3 ? item.serviceProductIdLevel3 : '',
          serviceProductIdLevel4: item.serviceProductIdLevel4 ? item.serviceProductIdLevel4 : '',
          serviceProductIdLevel5: item.serviceProductIdLevel5 ? item.serviceProductIdLevel5 : '',
          serviceProductIdLevel6: item.serviceProductIdLevel6 ? item.serviceProductIdLevel6 : '',
          servicePackageId: item.servicePackageId ? item.servicePackageId : '',
          servicePackageProductId: item.servicePackageProductId ? item.servicePackageProductId : '',
          serviceProductId: item.serviceProductId ? item.serviceProductId : '',
          serviceTypeId: item.serviceTypeId ? item.serviceTypeId : '',
          serviceTypeName: item.serviceTypeName ? item.serviceTypeName : ''
        }
        console.log(this.selectData)
        this.showBindserver = true;
      },
