        ...
        <div class="contanternews">
          <div class="selectChannel backWhite padding20 baseInfo">
            <div class="dataGenetalTitle">
              <span class="colorblack font16 fontWeight marginright10">首页图文广告</span>
            </div>
            <div class="padding20 colorGrey">最多添加一个广告</div>
            <div class="navigationBox flexBetween">
              <div class="tableBox">
                <table v-if="noData" class="table">
                  ...
                  <tr v-for="item in tableData" :key="item.id">
                    ...
                    <td>
                      <span v-if="item.isEmplyeeVisit == 0"> 否</span>
                      <span v-if="item.isEmplyeeVisit == 1"> 是</span>
                    </td>
                    <td>
                      <span class="color2087 font12 cursor" v-show="activityBannerDelete" @click="deleteItem(item.id)">删除</span>
                    </td>
                  </tr>
                </table>
              ...
              <div class="imgUrlBox" v-show="activityBannerAdd">
                <div class="lableBox">
                ...
                <div class="lableBox flex">
                  ...
                  <span class="colorGrey font12" style="margin-top: 45px;">675*172像素</span>
                </div>
                <div class="lableBox">
                ...
                <div class="lableBox">
                  <span class="colorGrey font12 marginright10">仅员工可见</span>
                  <i class="iconfont iconxuanzhong color2087 font20 cursor marginLeft10" v-if="isEmplyeeVisit==0"></i>
                  <i class="iconfont iconxuanze  font20 cursor marginLeft10" v-if="isEmplyeeVisit!=0" @click="isEmplyeeVisit = 0"></i>
                  <span class="typeText colorGrey font12 marginright10">否</span>
                  <i class="iconfont iconxuanzhong color2087 font20 cursor cursor"  v-if="isEmplyeeVisit == 1"></i>
                  <i class="iconfont iconxuanze  font20 cursor"   v-if="isEmplyeeVisit != 1" @click="isEmplyeeVisit = 1"></i>
                  <span class="colorGrey font12 marginright10">是</span>
                </div>
                <div class="smallButon addnavigation cursor" @click="addnavigation()">添加图文广告</div>
              </div>
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: '', name: '仅员工可见', showBlue: false, orderField: ''},
        ...
        title: '',
        noData: false,
        activityBannerAdd: false,
        activityBannerDelete: false,
      };
    },
    created() {
      var tokenVal = Util.localStorageUtil.get("access_token");
      this.headers = {token: tokenVal, shopId: 1};
      this.activityBannerAdd = this.havePermissions('activityBanner:add')
      this.activityBannerDelete = this.havePermissions('activityBanner:delete')
    },
    watch: {},
    mounted() {
    ...
    methods: {
      ...
      deleteItem(id) {
        this.$confirm('确认删除?', '', {
          ...
          Service.activitybanner().deletebanner({}, id).then(response => {
      ...
      addnavigation() {
        ...
        Service.activitybanner().addBanner({
          title: this.title,
          imgUrl: this.imgUrl,
          linkUrl: this.linkUrl,
          isEmplyeeVisit: this.isEmplyeeVisit
        }).then(
          (response) => {
            if (response.errorCode == 0) {
              this.getnavigationList();
      ...
      getnavigationList() {
        Service.activitybanner().getBannerList({
          page: 1,
          size: 100
        }).then(response => {
          if (response.errorCode == 0) {
