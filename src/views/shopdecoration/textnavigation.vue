      ...
      <div class="content">
        <div class="contanternews">
          <div class="selectChannel backWhite padding20 baseInfo">
            ...
            <div class="padding20 colorGrey">首页最多显示 8个导航，每行4个导航</div>
            <div class="navigationBox flexBetween">
              <div class="tableBox">
                <table v-if="noData" class="table">
                  ...
                  <tr v-for="(item,index) in tableData" :key="item.id">
                    <td><img :src="item.imgUrl" @click="showBig(item)" class="imgurlBox cursor"/></td>
                    <td>
                      <div class="font2">{{item.title}}</div>
                      <div class="colorGrey font2">{{item.linkUrl}}</div>
                    </td>
                    <td v-show="navigationSort">
                    ...
                    <td>
                      <span class="color2087 font12 cursor" v-if="item.isHomepageShow&&navigationShow"
                            @click="cancleHome(item.id)">取消首页</span>
                      <span class="color2087 font12 cursor" v-if="!item.isHomepageShow&&navigationShow"
                            @click="cancleHome(item.id)">首页展示</span>
                      <span class="line"></span>

                      <span class="color2087 font12 cursor" @click="deleteItem(item.id)"
                            v-show="navigationDelete">删除</span>
                      <span class="line"></span>
                      <span class="color2087 font12 cursor" @click="editItem(item.id)"
                            v-show="navigationUpdate">编辑</span>
                    </td>
                  </tr>
                </table>
              ...
              <div class="imgUrlBox" v-show="navigationAdd">
                <div class="lableBox">
                  <span class="colorGrey font12 marginright10">导航名称</span>
                  <input type="text" class="colorblack font12 inputBox" @change="titleChange()" v-model="title"
                         placeholder="请输入不过4个字导航名称"/>
                </div>
                    ...
                    <div class="applicaninfoone">
                      ...
                      <el-upload
                        ...
                        <div class="uploadtext">
                          <i v-if="!imgUrl" class="iconfont iconiconjia uploadIcon"></i>
                          <span v-if="!imgUrl" class="textwarm">上传照片</span>
                        </div>
                      </el-upload>
                    </div>
                  </div>
                  <span class="colorGrey font12" style="margin-top: 80px;">100*100像素</span>
                </div>
                <div class="lableBox">
                ...
                <div class="smallButon addnavigation cursor" @click="addnavigation()">{{buttonWord}}</div>
              </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        title: '',
        noData: false,
        navigationSort: false,
        navigationShow: false,
        navigationAdd: false,
        navigationDelete: false,
    ...
    created() {
      ...
      this.navigationSort = this.havePermissions('navigation:sort')
      this.navigationShow = this.havePermissions('navigation:show')
      this.navigationAdd = this.havePermissions('navigation:add')
      this.navigationDelete = this.havePermissions('navigation:delete')
      this.navigationUpdate = this.havePermissions('navigation:update')
    },
    watch: {
      editId(val) {
        if (val) {
          this.buttonWord = '保存图文导航'
        } else {
          this.buttonWord = '添加图文导航'
        }
      }
    },
    destroyed () {
    ...
    methods: {
      ...
      //   取消首页
      cancleHome(id) {
        Service.navigation().shownavigation({}, id).then(response => {
      ...
      sortRecData(firstId, secondId) {
        ...
        Service.navigation().sortnavigationlist({}, str).then(response => {
          if (response.errorCode == 0) {
            this.getnavigationList();
          } else {
            this.$message.error(response.message)
      ...
      deleteItem(id) {
        this.$confirm('确认删除?', '', {
          ...
          Service.navigation().deletenavigation({}, id).then(response => {
            if (response.errorCode == 0) {
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      async editItem(id) {
        ...
        this.editId = id
        const resDetails = await Service.navigation().navigationdetail({}, id)
        if (resDetails.errorCode != 0) {
          this.$message.error(resDetails.message)
          return
        }
        this.editparams = resDetails.data;
        this.title = resDetails.data.title;
        this.imgUrl = resDetails.data.imgUrl;
        this.linkUrl = resDetails.data.linkUrl;
        this.buttonWord = '保存图文详情';
      },
      changeLinkUrl(type) {
        var on = true;
      ...
      addnavigation() {
        ...
        if (this.editId) {
          let {sort, isHomepageShow, id} = this.editparams
          Service.navigation().editbanner({
            title: this.title,
            imgUrl: this.imgUrl,
      ...
      titleChange(type) {
        ...
        if (this.title.length > 4) {
          ...
          this.$message.error("请输入不超过4个字不低于2个字的店铺名称")
          on = false;
      ...
      getnavigationList() {
        Service.navigation().getnavigationlist({}).then(response => {
          if (response.errorCode == 0) {
            if (!response.data) {
              this.noData = false;
            } else {
