      ...
      <div class="content">
        <div class="contanternews">
          <div class="dataGeneral  backWhite padding20"  v-show="realTimesurvey">
            <div class="fontWeight font16 colorblack">乐兹实时概况
            </div>
            <table class="tableBox">
              ...
              <tr>
                ...
                <td>{{contentData.redeemOrderNums}}</td>
                <td>{{contentData.redeemTotalPoints}}</td>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        sortDatas: [
          ...
          {orderType: '', name: '（今日）兑换订单数', showBlue: false, orderField: ''},
          {orderType: '', name: '（今日）实际消耗积分', showBlue: false, orderField: ''},
          {orderType: '', name: '（今日）兑换人数', showBlue: false, orderField: ''}
        ],
        contentData: {},
        erchartData: [],
        xdata: [],
    ...
    created() {
      ...
      this.realTimesurvey = this.havePermissions('realTime:survey[shopId=2]')
      this.prehoursurvey = this.havePermissions('prehour:survey[shopId=2]')
    },
    mounted() {
    ...
    methods: {
      async getTotalData(statsTime){
        ...
        if (totalDate.errorCode != 0) {this.$message.error(todayDate.message)}
      ...
      async getDataList(statsTime, yesterday) {
        ...
        if (todayDate.errorCode != 0) {this.$message.error(todayDate.message)}
